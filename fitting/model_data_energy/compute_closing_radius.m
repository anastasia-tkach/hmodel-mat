...
if settings.energy3x == false && settings.energy3y == false && settings.energy3z == false
    return
end
...
for p = 1:length(poses)
    ...
    for v = 1:length(view_axes)
        ...
        [~, camera_axis, camera_center] = get_raytracing_matrix(poses{p}.centers, radii, poses{p}.data_bounding_box, view_axis, settings, 'front');
        closing_radius = 0;
