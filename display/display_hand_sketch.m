function [] = display_hand_sketch(poses, radii, blocks, invalid_blocks)
...
for p = 1:length(poses)
    if isfield(poses{p}, 'new_centers')
        centers = poses{p}.new_centers;
    else centers = poses{p}.centers;
    end
    ...
    for i = 1:length(centers)
        ...
        mypoint(centers{i}(axis_indices), 'b');
    end
