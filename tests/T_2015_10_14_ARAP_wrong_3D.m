...
for i = 1:length(blocks)
    if length(blocks{i}) == 2,
        distances{i} = [norm(centers{effector_indices{i}(1)} - centers{base_indices{i}})];
        edge_indices{i}{1} = [base_indices{i}; effector_indices{i}(1)];
    end
    if length(blocks{i}) == 3,
        distances{i}(1) = norm(centers{effector_indices{i}(1)} - centers{base_indices{i}});
        edge_indices{i}{1} = [base_indices{i}; effector_indices{i}(1)];
        distances{i}(2) = norm(centers{effector_indices{i}(2)} - centers{base_indices{i}});
        edge_indices{i}{2} = [base_indices{i}; effector_indices{i}(2)];
    end
end
...
while true
    ...
    for i = 1:length(points)
        c = centers{base_indices{block_indices{i}}};
    ...
    for i = 1:length(edge_indices)
        ...
        for j = 1:length(edge_indices{i})
            edge = (centers{edge_indices{i}{j}(2)} - centers{edge_indices{i}{j}(1)}) / ...
                norm(centers{edge_indices{i}{j}(2)} - centers{edge_indices{i}{j}(1)});
    ...
    for i = 1:length(blocks)  
        for j = 1:length(edge_indices{i})
            myline(centers{edge_indices{i}{j}(1)}, centers{edge_indices{i}{j}(1)} + edges{k}, 'm');
            k = k + 1;
        end
    ...
    for i = 1:length(points)
        index = base_indices{block_indices{i}};
        c = centers{index};
        p = points{i}; q = projections{i};
        d = q - c;
