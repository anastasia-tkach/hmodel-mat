function [valid_update, poses, radii] = apply_update(poses, blocks, radii, delta, D)

valid_update = false;
num_poses = length(poses);
num_centers = length(poses{1}.centers);
count = 0;
while (valid_update == false && count < 10)
    %% Update
    count = count + 1;
        ...
        for b = 1:length(blocks)
            ...
            if length(blocks{b}) == 3
                ...
                if has_tangent_plane1 == false || has_tangent_plane2 == false || has_tangent_plane3 == false || ...
                    ...
                    disp(['pose ', num2str(p), ', block ', num2str(b), ' - no tangent plane or cone']);
                    valid_update = false;
